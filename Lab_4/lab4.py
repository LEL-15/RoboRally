...
def convert_ultrasonic_to_robot_coords(x_us):
    ...
    global servo, pose2d_sparki_odometry
    x_r = (x_us*math.cos(math.radians(servo)))
    y_r = (x_us*math.sin(math.radians(servo)))
    #x_r, y_r = 0., 0.
    return x_r, y_r
...
def convert_robot_coords_to_world(x_r, y_r):
    ...
    x_w = 33.333*(pose2d_sparki_odometry.x + x_r*math.cos(theta) - y_r*math.sin(theta))
    y_w = 33.333*(pose2d_sparki_odometry.y + x_r*math.sin(theta) + y_r*math.cos(theta))
...
def populate_map_from_ping(ping_distance):
    ...
    #print("X: ", int(math.floor(worldLocX/3)))
    #print("Old Y: ", worldLocY)
    #print("Y: ", int(13 - math.floor(worldLocY/3)))
    testMap[int(19 - math.floor(worldLocY/2))] [int(math.floor(worldLocX/2))] = 1
    return

def display_map():
    #ATTEMPTED: Display the map
    """
    spark = ij_to_cell_index(pose2d_sparki_odometry.x*33.333, pose2d_sparki_odometry.y*33.333)
    lines = []
    line = ""
    ...
    """
    print(math.floor(20 - pose2d_sparki_odometry.y*16.6665))
    spark = [int(math.floor(pose2d_sparki_odometry.x*16.6665)), int(math.floor(20 - pose2d_sparki_odometry.y*16.6665))]
    for i in range(20):
        line = ""
        for j in range(30):
            if i == spark[1] and j == spark[0]:
                line = line + "S " 
            elif testMap[i][j] == 1:
                line = line + "O "
            else:
...
def ij_to_cell_index(i,j):
    ...
    xCell = math.floor(i/2)
    yCell = 13 - math.floor(j/2)
    return (xCell + yCell)#changed so it wasn't yCell*3
